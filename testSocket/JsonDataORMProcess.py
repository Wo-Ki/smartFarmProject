...
class JsonDataORMCtrl(object):
    @staticmethod
    def devicesJsonData(jsonData, clientSocket, lock):
        """处理设备的json数据"""
        ...
        if jsonData["M"] == 'checkin':
            ...
            sourceID = jsonData["SID"]
            find = session.query(DevicesTable).filter_by(ID=jsonData["TID"]).first()
